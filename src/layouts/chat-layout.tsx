...
import { saveConversationKeys } from "@/lib/conversation-store";
...
export default function ChatLayout() {
  ...
  const { mutateAsync: initiateGroupConversation } =
    useInitiateGroupConversation();
  ...
  const handleCreateGroupChat = async (name: string, userIds: number[]) => {
    ...
    for (const userId of userIds) {
      ...
      const { sharedKey, ephemeralKeyPublicJWK, usedOPKId } =
        await initializeX3DHSession(userBundle);
  ...
  const handleDeleteUser = async () => {
    ...
    if (userDeleted) {
      ...
      // No need to clear localStorage since we're using cookies now
      queryClient.clear();
